...
var adjust = $('.player .button .sounds .adjust ');
var adjust_cover = $('.player .button .sounds .adjust .vo_cover');
//播放列表
var playing_list = [];
...
//音量调节
$('.player .button .sound').on('click', function (e) {
    adjust.toggle();
});

adjust.on('click', function (e) {
    let adjust_left = adjust.offset().left;
    let adjust_width = adjust.width();
    let volume_num = (e.clientX - adjust_left) / adjust_width;
    if (volume_num < 0)
        volume_num = 0;
    audio.volume = volume_num;
    adjust_cover.width(e.clientX - adjust_left);
});
