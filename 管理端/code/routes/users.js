...
router.get('/comment_add', function (req, res, next) {
  ...
  db.query(str1, (err, ress) => {
    ...
    let str2 = 'insert into comment(com_id, mu_id, user_name, time, content, love)' +
      'values(' + (ress[0].num + 1) + ',' + s.mu_id + ',"' + s.user_name + '","' + s.content + '","' + s.time + '",' + 0 + ')';
    console.log(str2);
...
//admin page
router.get('/music-data', function (req, res, next) {
  console.log('进入music-data接口');
...
router.put('/music-data', function (req, res, next) {
  console.log('进入add-music-data接口');
  const s = req.body;
  let sql = 'insert into music (mu_id, name, singer, class, word, url, img_url, state) values ("' +
    s.mu_id + '","' + s.name + '","' + s.singer + '","' + s.class + '","' + s.word + '","' +
    s.url + '","' + s.img_url + '",' + s.state + ')';
  db.query(sql, (err, result) => {
    if (err) {
      res.send({
        code: 1,
...
router.post('/music-data', function (req, res, next) {
  console.log('进入eidt-music-data接口');
  const s = req.body;
  let sql = 'update music set name ="' + s.name + '", singer = "' + s.singer + '", class = "' + s.class +
    '", word = "' + s.word + '", url = "' + s.url + '", img_url = "' + s.img_url + '", state = ' + s.state +
    ' where mu_id =' + s.mu_id;
  db.query(sql, (err, result) => {
    if (err) {
      res.send({
...
router.delete('/music-data', function (req, res, next) {
  console.log('进入del-music-data接口');
  const s = req.query;
  if (s.del == 1) {
    let sql = 'delete from music where mu_id = "' + s.id + '"';
    db.query(sql, (err, result) => {
      if (err) {
        res.send({
          code: 1,
...
router.put('/user-data', function (req, res, next) {
  console.log('进入add-user-data接口');
  const s = req.body;
  let sql = 'insert into user (user_name, password, telephone, gender) values (' +
    '"' + s.user_name + '","' + s.password + '","' + s.telephone + '","' + s.gender + '")';
    console.log(sql);
  db.query(sql, (err, result) => {
    if (err) {
      res.send({
...
router.delete('/user-data', function (req, res, next) {
  console.log('进入del-user-data接口');
  const s = req.query;
  if (s.del == 1) {
    let sql = 'delete from user where user_name = "' + s.user_name + '"';
    db.query(sql, (err, result) => {
      if (err) {
        res.send({
          code: 1,
